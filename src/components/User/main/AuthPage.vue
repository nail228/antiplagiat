<template>
  <form class="auth-card" @submit.prevent="SubmitHandler"  method="POST">
...
import {useSubmit} from "@/components/User/main/hooks/Submit";
...
export default {
  ...
  setup () {
    ...
    async function SubmitHandler(){
      if (!form.valid){
        ...
        try{
          ...
          const isAdmin=await store.dispatch('isAdmin',formData)
    ...
    return { form ,SubmitHandler,submited}
  },
/*  data:()=>({
      email:'',
      password:''
